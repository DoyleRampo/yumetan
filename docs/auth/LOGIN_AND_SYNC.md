...
- ログアウト後に同じアカウントでログインすると、キャッシュとクラウドの同期結果を表示してアプリ本体（ホーム）から再開する。診断が未完了なら診断、プロフィールが無ければ登録画面から続ける。設定からログイン方法を追加（連携）したときだけ設定画面に留まる。

## アカウント削除

設定の「アカウント・同期」→「アカウント削除」から本人が削除できる。二段階の確認後に実行し、取り消しはできない。

1. サーバーの `POST /api/account/delete` がFirebaseトークンを検証し、Admin権限で次を削除する。
   - `users/{uid}` 配下すべて（プロフィール・夢・日記・写真・削除履歴）
   - コミュニティに公開した自分の投稿（コメント・リアクションを含む）、`communityStats/{uid}`、`communityBlocks/{uid}`、`memberships/{uid}`
   - Firebase Authenticationのユーザー本体（ログインIDそのもの）
2. サーバー未設定の配信では、本人限定のFirestoreルールの範囲でクライアントが自分の `users/{uid}` を消し、FirebaseのユーザーIDを削除する。この経路ではコミュニティ投稿は消えないため、サーバーを設定して運用する。
3. 端末からは `yumetan.v4.options`（言語・アラーム時刻・接続先URLなどの端末設定）以外のユメタン用キーをすべて削除する。記録・プロフィール・診断の途中経過・ゲストキャッシュ・未完了の認証セッションは残らない。
4. 削除後は匿名ユーザーに戻り、登録画面から再開する。次に新しくログインするときは、はじめて使う人と同じ手順（プロフィール登録 → 16問の診断 → アプリ開始）を必ず通る。以前のキャラクターや記録が復活することはない。

購読（RevenueCat）の解約はストア側の操作が必要。削除してもストアの定期購読は自動では止まらない。
