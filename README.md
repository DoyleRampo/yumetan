...
ブラウザテストではFirebase通信を無効化し、利用者の記録に触れずに、4言語登録・16問診断・リロード・日付ごとの夢と日記・睡眠レベル・画像添付・削除・タブバーのスワイプ・AI診断カード・AIエラー・オフライン動作を検証します。AIへの有料実リクエストはテストしません。画像OCRと生成結果はモックのAPI契約テストで検証します。
