...
- 投稿の内容チェックはありません（夢には落下・災害・死などが普通に出てくるため、判定サービスが誤って拒否していた）。公開本文は 20,000 文字まで。通報・ブロック・コメントの機能はありません。運営者は Firestore で `communityPosts/{id}.hidden=true` を設定すると投稿を非表示にでき、`memberships/{uid}.suspended=true` で作者を利用停止にできます。
...
- Webhook が届かなかった更新（無料インスタンスのスリープ中、配信の打ち切り、数分ごとに更新される Sandbox など）で有料会員がフリーに戻らないよう、`GET /api/account` は有料期間が切れた後の30分間、1分に1回までRevenueCatに照会し直します（期限直後の照会で更新がまだ見えず一度フリーと記録しても、この間に更新が見えれば戻ります）。それ以外は6時間に1回です。照会するのはRevenueCatに一度でも同期済みの会員だけです（未登録のIDを照会するとRevenueCat側に購読者が作られるため）。
...
- ストアアプリでは、起動時・アプリに戻ったとき（1分に1回まで）・プラン画面を開いたときに、端末のRevenueCat SDKからその時点の契約（`getCustomerInfo`）を読み直します。解約・期限切れ・ストア設定でのプラン変更もここで分かり、端末に覚えているプランを置き換えます。ストアとサーバーの答えが違えば、すぐに `POST /api/billing/sync` でサーバーにRevenueCatを照会させます。
- プラン画面の「現在のプラン」の印は、設定・記録枠・キャラクターと同じプラン（サーバーの確認済みプラン、またはストアが確認済みでサーバーが未反映の上位プラン）に付きます。サーバー未反映のあいだは「サーバーへの反映を待っています」と「もう一度確認」ボタンを、解約予定のときは「◯日まで使え、その後フリーに戻る」を表示します。
...
- 利用規約 `public/legal/terms.html` とプライバシーポリシー `public/legal/privacy.html` は API サーバーから配信される。App Store Connect の「プライバシーポリシー URL」に `https://<サーバー>/legal/privacy.html` を登録する。
