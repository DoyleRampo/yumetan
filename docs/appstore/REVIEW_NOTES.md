...
2. Introduction → create an account with email (`review-free`) → nickname → answer the 24 questions → character result.
3. Record a dream → "Read this dream" → save → reopen it from its date.
4. Settings → Plans → Starter → Details → tap "Terms of Use" and "Privacy Policy" → back → purchase button → sandbox purchase sheet → confirm → plan shows as current.
5. Sign out → sign in as `review-standard` → members' feed → open a post → add a stamp.
6. Open one of your own dreams → toggle "Publish to the feed" on, then off.
7. Settings → Account → Delete account → confirm → app returns to the first screen.
