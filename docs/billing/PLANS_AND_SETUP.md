...
- 公開は夢ごとに決めます。診断結果ページの「この夢をみんなの夢に公開する」（有料会員は初期状態でオン）にチェックを入れて保存すると、夢の本文がそのままニックネーム・タイプ・キャラクターとともにみんなの夢へ投稿されます（有料プランのみ。フリーではチェックボックスが無効でプラン案内を表示）。チェックを外して保存すると非公開に戻ります。公開状態は記録の `shared` フラグとしてアカウントに同期します。日次の公開枠超過や判定サービスの失敗で投稿できなかった場合は、夢の保存だけを行い、チェックを外してエラーを表示します。
...
- 支払期間終了/支払失敗後は、その作者の投稿も他ユーザーから非表示になります。有料状態に戻ると公開設定のままの投稿は再び見えます。無料に戻った本人も夢の詳細 → 公開設定から非公開にできます。
