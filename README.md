...
1. 呼び名・年代・言語を入力し、24問（頻度12問・場面4問・見かた8問）に回答。中断したアンケートは端末内で再開できます。
...
- `public/legal/terms.html`（利用規約）と `public/legal/privacy.html`（プライバシーポリシー）を日本語・英語で配信します（`?lang=ja|en`）。ヘルプページとプラン画面の「利用規約」「プライバシーポリシー」は `legalUrl()`（`public/core/help-content.js`）で `API_URL/legal/…?lang=` を開きます。事業者名・連絡先・対象年齢（13歳以上）は記載済みで、変更するときは両ファイルの日本語・英語セクションを直してください。
...
- 審査対応の計画は [docs/appstore/REVIEW_2_1_INFORMATION_NEEDED_PLAN.md](docs/appstore/REVIEW_2_1_INFORMATION_NEEDED_PLAN.md)、本人が行う作業は [docs/appstore/OWNER_TASKS.md](docs/appstore/OWNER_TASKS.md)、Apple への返信文は [docs/appstore/REVIEW_NOTES.md](docs/appstore/REVIEW_NOTES.md)。「みんなの夢」の通報・ブロックは削除のままとし、審査への説明で対応します（Guideline 1.2 で指摘されたら `main` の実装を移植）。
...
- `GET /api/health`：接続設定の有無と `aiStatus`（設定したキーでモデルが見えるか。秘密値は返しません）。`authConfigured` が false ならサーバーに Firebase Admin の認証情報（`FIREBASE_SERVICE_ACCOUNT_JSON`）が無く、アプリ内ログインは失敗します
...
`public/core/diagnosis.js`（設問・採点）と `public/core/type-lore.js`（16タイプの根拠とストーリー）。設計の詳細は [knowledge/dream_types16.md](knowledge/dream_types16.md)。

診断は「4つのグループ軸 × 4つのスタイル軸 × 場面選択」の24問です（v2）。

| パート | 問数 | 形式 | 測るもの |
|---|---|---|---|
| 頻度 | 12 | 5段階の頻度尺度（まったくない〜週に何度も） | 悪夢・予知・明晰・反復の4グループ軸を各3問 |
| 場面 | 4 | 4択のシナリオ | 各グループの中で、どのタイプの場面がいちばん近いか |
| 見かた | 8 | 両極5段階（かなり／やや／どちらとも） | 「動く↔見つめる」「外の世界↔自分の内側」「はっきりした場面↔雰囲気・気配」「これから↔見覚え」の4スタイル軸を各2問 |

各グループは2つのスタイル軸で2×2に分かれ、16タイプそれぞれに固有の極の組み合わせがあります（例: 追跡系 = 悪夢 × 動く × 外の世界）。16タイプすべてのスコアを0〜100で計算します。

- グループ軸（0〜1）× 50点
- スタイル軸との一致度（-1〜1を0〜1に変換）× 30点
- 場面の答えが一致 +20点
- 直近12件の夢に付いたテーマは、1件・1テーマにつき +8点。日記は投票しません。

最高得点のタイプを採用。同点は「明晰 → 反復 → 予知 → 悪夢」のグループ順、次に図鑑順で決め、頻度がすべて0のときは仮のタイプと表示します（結果が悪夢グループに偏らない設計）。結果画面には、4軸のバランス、スタイル軸上の位置、場面の答え、夢の記録件数、次に近いタイプを根拠として表示します。旧16問（v1）の回答も引き続き判定でき、ホームから新しい診断へ誘導します。各夢のテーマは手動選択が優先、未選択なら4言語のキーワードで推定します。キーワード処理は否定や文脈を完全には理解しないため、ユーザーがテーマを修正できます。
...
ブラウザテストではFirebase通信を無効化し、利用者の記録に触れずに、4言語登録・24問診断・リロード・日付ごとの夢と日記・夢レベル・戻るリンクと左端スワイプ・画像添付・削除・タブバーのスワイプ・AI診断カード・AIエラー・オフライン動作を検証します。AIへの有料実リクエストはテストしません。画像OCRと生成結果はモックのAPI契約テストで検証します。
