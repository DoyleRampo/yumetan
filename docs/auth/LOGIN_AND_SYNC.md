...
- ログアウト後に同じアカウントでログインすると、それまでの記録を同期して表示し、ホームからアプリを再開する。

## アカウント削除

設定 →「アカウント削除」。確認後に実行し、取り消せない。

- サーバーの `POST /api/account/delete` が、`users/{uid}` 配下・公開した投稿とそのコメント/リアクション・`memberships/{uid}`・`communityStats/{uid}`・当月の使用量、そしてFirebase Authenticationのユーザー本体を削除する。
- サーバーに届かないときだけ、クライアントが自分の `users/{uid}` の記録・日記・削除履歴・プロフィールを消してからFirebaseのユーザーを削除する。この経路は直近のログインが必要なので、古いログインでは何も消さずに「もう一度ログインしてから削除」と案内する（中途半端に消さない）。コミュニティ投稿はサーバー経由でのみ削除できる。
- 端末側は、削除したアカウントのキャッシュだけでなく、以前のアカウントやゲストが残した領域も含めて `yumetan.` で始まるキーをすべて削除する（端末設定 `yumetan.v4.options` のみ残す）。古い領域が残っていると、匿名UIDを取得できない環境では共有の `yumetan.v4.local` に戻り、その古いプロフィールが次のログインに引き継がれて登録と診断が飛ばされる。
- 削除後は最初のログイン画面へ戻る。次に新しくログインしたときは、はじめて使う人と同じく登録 → 診断アンケート → ホームの順に進む。以前のキャラクターや記録は復活しない。
- ストアの定期購読（RevenueCat）は自動では解約されない。解約はApp Store / Google Play側で行う。
