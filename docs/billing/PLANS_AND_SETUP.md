...
| 手書きOCR（月） | なし | 5回 | 20回 |
| みんなの夢 | その日の投稿からランダム3件・冒頭20文字のみ | 30件 / 日 | 150件 / 日 |
| 新しく公開する夢（日） | なし | 1件 | 3件 |
| 同時公開数 | なし | 10件 | 50件 |
| コメント（日） | なし | 10件 | 30件 |
...
フリー会員の「みんなの夢」は `GET /api/community/teaser?day=YYYY-MM-DD&tz=分` で、その日に公開された投稿（有料会員の投稿のうち非表示・ブロック対象を除く）から、利用者IDと日付から決まる乱数で3件を選び、本文の先頭20文字だけを返します。同じ日に何度読み込んでも同じ3件で、全文は返しません。「…続きを読む」「もっと見る」はプラン画面へ遷移します。
...
通常の読み解きを入力6,000/出力1,400トークンと仮定すると1回約0.46円（入力0.19円＋出力0.27円）、月30回約14円、90回約42円。長文・OCRに余裕を残しつつ内部上限を設けています。利益は保証できないため、実際の入力量・会員数・解約率・Firebase使用量で月次レビューしてください。無制限のGPTや無制限の投稿閲覧は設定していません。
