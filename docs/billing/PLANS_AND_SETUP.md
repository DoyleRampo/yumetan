...
夢を公開することは全プランでできます。有料プランで開くのは「他の人の夢を読むこと」（全文・スタンプ・コメント）です。
...
自分が公開した投稿は、この一覧に「自分」の印つきで全文のまま並びます（3件の枠外、最新20件）。自分の投稿はフリー会員でも詳細を開けて、届いたコメントを読めます。スタンプとコメントの投稿は有料プランのままです。
...
投稿表示は一覧で返した件数＋詳細表示回数。再読み込み、コメント/スタンプ後の詳細再取得も表示枠を使います。自分の公開投稿は「みんなの夢」の一覧にも「自分の投稿」の印つきで並びますが、表示枠は使いません（自分の投稿の管理も対象外）。1投稿につき300コメント、1コメント500文字、公開本文4,000文字、タイトル80文字、ニックネーム30文字。スタンプは1ユーザー1投稿につき1つで置換/取消可能。操作ごとに日次枠を使います。
