...
- 投稿/コメントはOpenAI Moderationで確認してから公開。判定サービスの失敗時は公開しません。通報・コメント削除に対応し、運営者による通報確認と非表示処理を用意しています。通報とブロックはプランを問わず誰でも使えます（フリー会員は抜粋カードからも通報・ブロックできます）。ブロックは `communityBlocks/{uid}/targets/{相手uid}` に記録し、どちらか一方がブロックしていれば双方の投稿・詳細・スタンプ・コメントを不可視にします。ブロックした一覧は「みんなの夢」から確認・解除できます。
